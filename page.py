...
# A Termux / mobile-friendly UA (used automatically when Termux detected)
TERMUX_UA = "Mozilla/5.0 (Linux; Android 11; Mobile; rv:120.0) Gecko/120.0 Firefox/120.0"
THREADS = 12
...
def run_mode_file(user_agent):
    ...
    fn = input(" ").strip()
    if not fn:
        cprint(f"{owner_badge()} {Fore.RED}No filename provided. Exiting file mode.")
        return
    if not os.path.isfile(fn):
        cprint(f"{owner_badge()} {Fore.RED}File not found: {fn}")
        return
...
def main():
    ...
    # Determine user-agent automatically for Termux, or use DEFAULT_UA
    user_agent = os.getenv('MMG_UA')
    for arg in sys.argv[1:]:
        if arg.startswith('--ua='):
            user_agent = arg.split('=', 1)[1]
            break

    if not user_agent:
        if detect_termux():
            user_agent = TERMUX_UA
            cprint(f"{owner_badge()} {Fore.CYAN}Termux detected — using mobile user-agent automatically.", Fore.WHITE)
        else:
            user_agent = DEFAULT_UA
            cprint(f"{owner_badge()} {Fore.CYAN}Using default desktop user-agent.", Fore.WHITE)

    cprint(f"{owner_badge()} {Fore.CYAN}User-Agent in use: {user_agent[:140]}", Fore.WHITE)
